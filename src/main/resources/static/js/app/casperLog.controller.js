/**
 * Casper contract parsed summary logs look-a-like
 *
 * logs component with features:
...
(function() {
    ...
    function CasperLogCtrl($scope, $timeout, scrollConfig) {
        // checkbox value
        ...
        $scope.$on('casperLogEvent', function(event, data) {
        ...
        $scope.$on('currentCasperLogs', function(event, items) {
            $scope.pendingLogs.forEach(addLogLine);
            $scope.pendingLogs = [];
        ...
        function resizeLogContainer() {
            console.log('Casper page resize');
    ...
    angular.module('HarmonyApp')
        .controller('CasperLogCtrl', ['$scope', '$timeout', 'scrollConfig', CasperLogCtrl]);
})();
